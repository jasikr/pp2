        ...
        if ball.colliderect(block):
            if block in unbreakable_blocks:
                ...
                if block in bonus_blocks:
                    bonus_ball = pygame.Rect(block.x + block.width // 2, block.y + block.height, ball_rect, ball_rect)
                    dx_bonus, dy_bonus = 1, 1
                    bonus_drops.append((bonus_ball, dx_bonus, dy_bonus))

    # Move and draw bonus balls
    for bonus_ball, dx_bonus, dy_bonus in bonus_drops[:]:
        bonus_ball.x += ballSpeed * dx_bonus
        bonus_ball.y += ballSpeed * dy_bonus
        pygame.draw.circle(screen, (0, 255, 255), bonus_ball.center, ballRadius)  # Bonus balls are cyan
        # Collision detection for bonus balls
        if bonus_ball.centerx < ballRadius or bonus_ball.centerx > W - ballRadius:
            dx_bonus = -dx_bonus
        if bonus_ball.centery < ballRadius or bonus_ball.centery > H - ballRadius:
            dy_bonus = -dy_bonus
        if bonus_ball.colliderect(paddle) and dy_bonus > 0:
            dx_bonus, dy_bonus = detect_collision(dx_bonus, dy_bonus, bonus_ball, paddle)
        for block in block_list[:]:
            if bonus_ball.colliderect(block):
                if block in unbreakable_blocks:
                    dx_bonus, dy_bonus = detect_collision(dx_bonus, dy_bonus, bonus_ball, block)
                else:
                    block_list.remove(block)
                    dx_bonus, dy_bonus = detect_collision(dx_bonus, dy_bonus, bonus_ball, block)
                    game_score += 1
